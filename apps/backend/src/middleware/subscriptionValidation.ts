  ...
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (enterpriseFeatures.includes(featureName) && user.tier !== 'enterprise') {
        logger.warn(`Unauthorized ${featureName} access attempt`, {
          userId,
          tier: user.tier,
          feature: featureName,
          endpoint: req.path,
        ...
        return res.status(403).json({
          success: false,
          message: `${featureName.replace('-', ' ')} requires an Enterprise subscription`,
  ...
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const rateLimits: { [key: string]: { [key: string]: number } } = {
        free: {
          'ai-career-coach': 0, // No access for free tier
          'ai-resume-builder': 0,
          'ai-cover-letter': 0
        },
        enterprise: {
