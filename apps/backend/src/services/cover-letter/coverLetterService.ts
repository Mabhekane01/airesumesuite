      ...
      if (enhanceWithAI && basicUpdateData.content) {
        ...
        if (originalCoverLetter) {
          try {
            ...
            const enhancePrompt = `
You are an Elite Executive Copywriter. Your task is to perform a strategic narrative enhancement of this cover letter while preserving its core institutional message.

CURRENT CONTENT:
${basicUpdateData.content}

STRATEGIC CONTEXT:
- Position Blueprint: ${originalCoverLetter.jobTitle}
- Target Organization: ${originalCoverLetter.companyName}
- Desired Tone: ${originalCoverLetter.tone}
- Primary Enhancement Focus: ${focusAreas?.join(', ') || 'Overall Strategic Impact'}

REQUIRED EXECUTION:
1. SEMANTIC POWER: Strengthen language to be more authoritative and results-oriented.
2. FLOW NODES: Improve logical transitions and narrative coherence.
3. VALUE PROP: Sharpen the connection between candidate mastery and organizational pain points.
4. ATS ARCHITECTURE: Ensure seamless integration of relevant industry keywords.

CRITICAL RULES:
- Use PLAIN TEXT ONLY. NO markdown formatting (**bold**, *italics*, etc).
- Return ONLY the enhanced content string. No explanations or metadata.
`;
...
CRITICAL: DO NOT use markdown formatting like **bold** or *italics* in your response. All text must be pure plain text only.
