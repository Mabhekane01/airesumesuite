...
export class LaTeXService {
  ...
  constructor(options: LaTeXServiceOptions = {}) {
    ...
    this.config = {
      ...
      latexCompiler: options.latexCompiler || "pdflatex",
  ...
  public async checkDependencies(): Promise<{
    ...
    // If local compiler is missing, check for Docker
    if (missing.includes(this.config.latexCompiler)) {
      console.log('⚠️ Local LaTeX compiler not found. Checking for Docker...');
      try {
        ...
        return {
          isReady: true,
          missing,
          texLiveInfo: { version: 'Docker', distribution: 'ghcr.io/mabhekane01/airesumesuite-latex-base' }
        };
      } catch (dockerError) {
        console.warn('❌ Docker not found either. Compilation will fail.');
      }
    }

    // Get TeX Live information if available locally
    if (!this.useDocker) {
      try {
...
${p.technologies ? `\\\\\\textit{Technologies:} ${escape(p.technologies.join(", "))}` : ""}`
  ...
  private async runCompiler(
    ...
    const latexSource = await fs.readFile(texFilePath, 'utf8');
    let command = this.detectRequiredCompiler(latexSource);
    let args: string[] = [];
    
    // Check if we need to initialize dependencies (detect docker if not done yet)
    if (!this.useDocker) {
        // Run a quick check just in case it wasn't initialized
        await this.checkDependencies();
    }

    if (this.useDocker) {
        console.log(`🐳 Using Docker for compilation: ${command}`);
        const compilerName = path.basename(command).replace('.exe', ''); // pdflatex or xelatex
        const dockerImage = 'ghcr.io/mabhekane01/airesumesuite-latex-base:latest';
        
        // Construct Docker command
        // Mount the compilation directory (cwd) to /workspace in the container
        // Fix for Windows: Replace backslashes with forward slashes for Docker volume mount
        let hostPath = cwd;
        if (process.platform === 'win32') {
            hostPath = hostPath.replace(/\\/g, '/');
        }
        const volumeMount = `${hostPath}:/workspace`;
        ...
        args = [
            ...
            'resume.tex' // Inside container, it's always resume.tex at root of /workspace
        ];
    } else {
        console.log(`🔧 Using local compiler: ${command}`);
        args = [
          "-interaction=nonstopmode",
          "-halt-on-error",
          "-output-directory",
    ...
    if (hasErrors) {
      ...
      // Don't throw immediately, let PDF check fail if file is empty
    }
