  ...
  return (
    <div className="min-h-screen bg-gradient-dark py-4 sm:py-6 lg:py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 sm:space-y-8">
        {/* Header */}
        <div className="glass-dark-heavy rounded-2xl p-6 sm:p-8 border border-dark-border/50 shadow-dark-xl">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            <div className="flex-1">
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-dark-text-primary gradient-text-dark">
                Welcome back, {user?.firstName}! 👋
              </h1>
              <p className="text-dark-text-secondary mt-2 text-sm sm:text-base">
                Here's what's happening with your job search today.
              </p>
            </div>
            <div className="hidden sm:flex items-center space-x-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-dark-text-primary">12</div>
                <div className="text-xs text-dark-text-muted">Applications</div>
              </div>
              <div className="w-px h-8 bg-dark-border"></div>
              <div className="text-center">
                <div className="text-2xl font-bold text-dark-text-primary">5</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {stats.map((stat, index) => {
            ...
            return (
              <div
                key={index}
                className="glass-dark rounded-xl p-4 sm:p-6 border border-dark-border/30 hover:border-accent-primary/50 backdrop-blur-sm shadow-dark-lg hover:shadow-glow-md transition-all duration-300 transform hover:scale-105 hover:-translate-y-1"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-medium text-dark-text-secondary truncate">{stat.title}</p>
                    <p className="text-xl sm:text-2xl font-bold text-dark-text-primary mt-1">{stat.value}</p>
        ...
        {/* Quick Actions */}
        <div className="glass-dark-heavy rounded-2xl p-6 sm:p-8 border border-dark-border/50 backdrop-blur-md shadow-dark-xl">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-dark-text-primary gradient-text-dark">Quick Actions</h2>
          </div>
            ...
            {quickActions.map((action, index) => {
              ...
              return (
                ...
                >
                  <Link
                    to={action.href}
                    className="group relative overflow-hidden glass-dark rounded-xl p-6 border border-dark-border/50 hover:border-accent-primary/50 transition-all duration-300 block shadow-dark-lg hover:shadow-glow-md backdrop-blur-sm"
                  >
                    {/* Gradient overlay */}
                    ...
                    <div className="relative z-10">
                      ...
                      <h3 className="text-lg font-semibold text-dark-text-primary mb-2 group-hover:text-accent-primary transition-colors duration-300">
                        {action.title}
                      </h3>
                      <p className="text-sm text-dark-text-secondary group-hover:text-dark-text-secondary transition-colors duration-300">
                        {action.description}
                      </p>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          {/* Recent Activity */}
          <div className="glass-dark rounded-xl p-6 border border-dark-border/50 backdrop-blur-sm shadow-dark-lg">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-dark-text-primary">Recent Activity</h2>
              <Link
                to="/activity"
            ...
            <div className="space-y-4">
              {recentActivity.map((activity, index) => (
                <div key={index} className="flex items-center space-x-4 p-3 rounded-lg hover:bg-dark-quaternary/50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-dark-text-primary">{activity.title}</p>
          ...
          <div className="glass-dark rounded-xl p-6 border border-dark-border/50 backdrop-blur-sm shadow-dark-lg">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-dark-text-primary">Upcoming Tasks</h2>
              <Link
                to="/tasks"
                className="text-accent-primary hover:text-accent-primary/80 text-sm font-medium flex items-center"
              >
            ...
            <div className="space-y-4">
              {upcomingTasks.map((task, index) => (
                <div key={index} className={`p-4 rounded-lg border-l-4 ${getPriorityColor(task.priority)}`}>
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-dark-text-primary">{task.title}</h3>
        ...
        <div className="glass-dark-heavy rounded-2xl p-6 sm:p-8 border border-dark-border/50 backdrop-blur-md shadow-dark-xl">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-dark-text-primary gradient-text-dark">Job Search Progress</h2>
          </div>
          <div className="h-48 sm:h-64 bg-gradient-to-br from-accent-primary/10 via-transparent to-accent-secondary/10 rounded-xl flex items-center justify-center border border-dark-border/30">
            <div className="text-center">
              ...
              <p className="text-xs sm:text-sm text-dark-text-secondary mt-2 max-w-sm mx-auto">Track your job search metrics and progress over time with detailed insights.</p>
            </div>
          </div>
        </div>
      </div>
