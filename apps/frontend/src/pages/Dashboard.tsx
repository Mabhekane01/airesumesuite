...
export default function Dashboard() {
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      default:
        return 'border-surface-200 bg-gray-700/30';
    }
  };
    ...
    <div className="min-h-screen bg-gradient-dark py-2 xs:py-3 sm:py-4 md:py-6 lg:py-8">
      <div className="max-w-7xl mx-auto px-2 xs:px-3 sm:px-4 md:px-6 lg:px-8 space-y-3 xs:space-y-4 sm:space-y-6 lg:space-y-8">
        ...
        <motion.div
          ...
          className="glass-dark-heavy rounded-lg xs:rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 lg:p-8 border border-surface-200/50 shadow-dark-xl"
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 xs:space-y-3 sm:space-y-0">
            <div className="flex-1 min-w-0">
              <h1 className="text-lg xs:text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-text-primary gradient-text-dark animate-gradient">
                Welcome back, {user?.firstName}! 👋
              </h1>
              <p className="text-text-secondary mt-1 xs:mt-1.5 sm:mt-2 text-xs xs:text-sm sm:text-base leading-relaxed">
                Here's what's happening with your job search today.
              </p>
            </div>
            <div className="flex sm:hidden items-center justify-center space-x-4 xs:space-x-6 pt-1 xs:pt-2">
              <div className="text-center">
                <div className="text-lg xs:text-xl font-bold text-text-primary">12</div>
                <div className="text-xs text-dark-text-muted">Applications</div>
              </div>
              <div className="w-px h-5 xs:h-6 bg-dark-border"></div>
              <div className="text-center">
                <div className="text-lg xs:text-xl font-bold text-text-primary">5</div>
                <div className="text-xs text-dark-text-muted">Interviews</div>
              </div>
            </div>
            <div className="hidden sm:flex items-center space-x-6">
              <div className="text-center">
                <div className="text-xl md:text-2xl lg:text-3xl font-bold text-text-primary">12</div>
                <div className="text-xs text-dark-text-muted">Applications</div>
              </div>
              <div className="w-px h-6 md:h-8 bg-dark-border/70"></div>
              <div className="text-center">
                <div className="text-xl md:text-2xl lg:text-3xl font-bold text-text-primary">5</div>
                <div className="text-xs text-dark-text-muted">Interviews</div>
              </div>
            </div>
          </div>
          ...
          {stats.map((stat, index) => {
            ...
            return (
              <motion.div
                ...
                className="glass-dark rounded-md xs:rounded-lg sm:rounded-xl p-2 xs:p-3 sm:p-4 lg:p-6 border border-surface-200/30 hover:border-accent-primary/50 backdrop-blur-sm shadow-dark-lg hover:shadow-glow-md transition-all duration-300 animate-slide-up-soft"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between space-y-1 xs:space-y-0">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs xs:text-sm font-medium text-text-secondary truncate">{stat.title}</p>
                    <p className="text-base xs:text-lg sm:text-xl lg:text-2xl font-bold text-text-primary mt-0.5 xs:mt-1">{stat.value}</p>
        ...
        <motion.div
          ...
          className="glass-dark-heavy rounded-lg xs:rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 lg:p-8 border border-surface-200/50 backdrop-blur-md shadow-dark-xl"
        >
                ...
                >
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      ...
                      <h3 className="text-sm xs:text-base sm:text-lg font-semibold text-text-primary mb-1 xs:mb-1.5 sm:mb-2 group-hover:text-accent-primary transition-colors duration-500 line-clamp-1">
                        {action.title}
                      </h3>
                      <p className="text-xs sm:text-sm text-dark-text-tertiary group-hover:text-text-secondary transition-colors duration-500 line-clamp-2">
                        {action.description}
                      </p>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 xs:gap-4 sm:gap-6 lg:gap-8">
          ...
          <motion.div
            ...
            className="glass-dark rounded-lg xs:rounded-xl p-3 xs:p-4 sm:p-5 lg:p-6 border border-surface-200/50 backdrop-blur-sm shadow-dark-lg"
          >
            <div className="flex items-center justify-between mb-3 xs:mb-4 sm:mb-5 lg:mb-6">
              <h2 className="text-base xs:text-lg sm:text-xl font-bold text-text-primary">📈 Recent Activity</h2>
              <Link
                to="/activity"
            ...
            <div className="space-y-2 xs:space-y-3 sm:space-y-4">
              {recentActivity.map((activity, index) => (
                <div key={index} className="flex items-center space-x-2 xs:space-x-3 sm:space-x-4 p-2 xs:p-2.5 sm:p-3 rounded-md xs:rounded-lg hover:bg-dark-quaternary/30 transition-all duration-300 border border-transparent hover:border-accent-primary/20 animate-slide-up-soft" style={{ animationDelay: `${index * 0.1}s` }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs xs:text-sm font-medium text-text-primary truncate">{activity.title}</p>
          ...
          >
            <div className="flex items-center justify-between mb-3 xs:mb-4 sm:mb-5 lg:mb-6">
              <h2 className="text-base xs:text-lg sm:text-xl font-bold text-text-primary">📋 Upcoming Tasks</h2>
              <Link
                to="/tasks"
                className="text-accent-primary hover:text-accent-secondary text-xs sm:text-sm font-medium flex items-center group"
              >
            ...
            <div className="space-y-2 xs:space-y-3 sm:space-y-4">
              {upcomingTasks.map((task, index) => (
                <div key={index} className={`p-2 xs:p-3 sm:p-4 rounded-md xs:rounded-lg border-l-3 xs:border-l-4 ${getPriorityColor(task.priority)} bg-dark-quaternary/20 animate-slide-up-soft`} style={{ animationDelay: `${index * 0.1}s` }}>
                  <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between space-y-1 xs:space-y-0">
                    <h3 className="text-xs xs:text-sm font-medium text-text-primary truncate pr-2">{task.title}</h3>
        ...
        >
          <div className="flex items-center space-x-2 xs:space-x-3 mb-3 xs:mb-4 sm:mb-6">
            ...
            <h2 className="text-base xs:text-lg sm:text-xl lg:text-2xl font-bold text-text-primary gradient-text-dark">Job Search Progress</h2>
          </div>
          <div className="h-32 xs:h-40 sm:h-48 lg:h-64 bg-gradient-to-br from-accent-primary/10 via-transparent to-accent-secondary/10 rounded-lg xs:rounded-xl flex items-center justify-center border border-surface-200/30">
            <div className="text-center px-3 xs:px-4">
              ...
              <p className="text-text-primary text-xs xs:text-sm sm:text-base font-medium">Analytics dashboard coming soon!</p>
              <p className="text-xs text-text-secondary mt-1 xs:mt-2 max-w-xs sm:max-w-sm mx-auto leading-relaxed">Track your job search metrics and progress over time with detailed insights.</p>
            </div>
          </div>
        </motion.div>
      </div>
