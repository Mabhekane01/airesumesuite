...
export default function ResumeDownloadManager({ 
  ...
  return (
    <div className={`space-y-6 animate-slide-up-soft ${className}`}>
          ...
          return (
            ...
            >
              ...
              <div className="space-y-3 mb-6">
                <div>
                  <p className="text-xs font-medium text-text-primary mb-1">Best for:</p>
                  <p className="text-xs text-text-secondary">{format.useCase}</p>
                </div>
                
                <div>
                  <p className="text-xs font-medium text-text-primary mb-1">Compatibility:</p>
                  <p className="text-xs text-text-secondary">{format.compatibility}</p>
                </div>
              </div>
            ...
            {aiData.savedPdfs.map((savedPdf) => (
              <Card key={savedPdf.id} className="card-dark p-4">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <h4 className="font-medium text-text-primary">{savedPdf.name}</h4>
      ...
      {showSaveDialog && (
        <div className="fixed inset-0 bg-white bg-opacity-50 flex items-center justify-center z-50 p-4">
          <Card className="card-dark p-6 max-w-md w-full">
            <h3 className="text-lg font-semibold text-text-primary mb-4">Save PDF to Library</h3>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-gray-700 border border-surface-200 rounded-lg text-text-primary placeholder-dark-text-muted focus:outline-none focus:border-dark-accent"
                />
              </div>
      ...
      {/* Usage Tips */}
      <Card className="glass-dark p-6 border border-surface-200">
      ...
      <Card className="glass-dark p-4 border border-surface-200">
        <div className="flex items-start">
          <CheckCircleIcon className="w-5 h-5 text-green-400 mr-2 mt-0.5" />
          <div>
            <p className="text-sm font-medium text-text-primary">ATS-Optimized PDF</p>
