...
const EnhancementFeedbackModal: React.FC<EnhancementFeedbackModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      `}>
        {/* Header */}
        <div className="sticky top-0 bg-surface-50/95 backdrop-blur-sm border-b border-surface-200 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-brand-dark">{result.title}</h2>
        ...
        <div className="p-6 space-y-6">
          {/* Score Improvement */}
          <div className="bg-surface-50/50 rounded-xl p-6 border border-surface-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {(result.keyMetrics || []).map((metric, index) => (
                <div key={index} className="bg-gray-700/50 rounded-lg p-4">
                  ...
                  <div className="text-xl font-bold text-brand-dark">{metric.value}</div>
                  {metric.improvement && (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {result.improvements.map((improvement, index) => (
                <div 
                  key={index} 
                  className="bg-surface-50/50 rounded-lg p-4 border border-surface-200"
                >
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-medium text-brand-dark">{improvement.category}</h4>
                  ...
                  {showDetails && (
                    <div className="space-y-1">
                      {improvement.changes.map((change, changeIndex) => (
                        <div key={changeIndex} className="flex items-start space-x-2 text-sm text-text-secondary">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-surface-200">
            ...
            {onViewComparison && (
              <button
                ...
                className="flex-1 bg-surface-50 hover:bg-gray-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 flex items-center justify-center space-x-2 border border-surface-200"
              >
            ...
            <button
              onClick={onClose}
              className="sm:w-auto px-6 py-3 text-text-secondary hover:text-brand-dark transition-colors"
            >
              Maybe Later
            </button>
          </div>
          ...
          <div className="bg-teal-500/10 border border-teal-400/30 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h4 className="font-medium text-brand-dark mb-1">Pro Tip</h4>
