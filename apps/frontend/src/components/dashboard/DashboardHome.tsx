      ...
      <div className="animate-pulse space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="h-24 sm:h-28 lg:h-32 bg-gray-700 rounded-lg"
            ></div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="glass-dark-heavy rounded-xl p-4 sm:p-6 text-white shadow-glow-md border border-accent-primary/30 backdrop-blur-lg relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
          <div className="flex-1 min-w-0">
            ...
            <p
              className="text-dark-text-secondary text-sm sm:text-base animate-slide-up-soft leading-relaxed"
              style={{ animationDelay: "0.1s" }}
            >
              You're making great progress on your career journey. Here's what's
              happening today.
            </p>
          </div>
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-lg sm:text-xl lg:text-2xl font-bold text-dark-text-primary mt-1">
                  {stat.value}
                </p>
              </div>
              <div
                className={`${stat.color} p-2 sm:p-2.5 lg:p-3 rounded-lg shadow-glow-sm flex-shrink-0 self-start sm:self-center`}
              >
      ...
      <div className="card-glass-dark p-4 sm:p-5 lg:p-6">
        <h2 className="text-lg sm:text-xl font-semibold text-dark-text-primary mb-4 sm:mb-6 gradient-text-dark">
          ⚡ Quick Actions
        </h2>
            ...
            >
              <div className="flex flex-col items-center text-center space-y-2 sm:space-y-3">
                <div className="relative">
                  ...
                  <div
                    className={`relative glass-dark-heavy p-3 sm:p-4 rounded-xl border border-accent-primary/30 group-hover:border-accent-primary/60 group-hover:shadow-glow-sm transition-all duration-500`}
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
        ...
        <div className="card-gradient-dark p-4 sm:p-5 lg:p-6">
          <div className="flex items-center space-x-2 mb-3 sm:mb-4">
            ...
            <h2 className="text-base sm:text-lg font-semibold text-dark-text-primary">
              🧠 AI Insights
            </h2>
          </div>
          <div className="space-y-2.5 sm:space-y-3">
            {aiInsights.map((insight, index) => (
              <div
                key={index}
                className="flex items-start space-x-2.5 sm:space-x-3 p-2.5 sm:p-3 bg-accent-primary/10 rounded-lg border border-accent-primary/20 animate-slide-up-soft"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
        ...
        {/* Recent Activity */}
        <div className="card-gradient-dark p-4 sm:p-5 lg:p-6">
          <h2 className="text-base sm:text-lg font-semibold text-dark-text-primary mb-3 sm:mb-4">
            📈 Recent Activity
          </h2>
        ...
        <div className="card-glass-dark p-4 sm:p-5 lg:p-6">
          ...
          <div className="space-y-2.5 sm:space-y-3">
            {upcomingEvents.slice(0, 3).map((event, index) => (
              <div
                key={event.id}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-2.5 sm:p-3 bg-dark-quaternary/30 rounded-lg border border-dark-border hover:border-accent-primary/30 transition-all duration-300 animate-slide-up-soft space-y-2 sm:space-y-0"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="flex items-center space-x-2.5 sm:space-x-3 min-w-0">
                  <CalendarIcon className="w-4 h-4 sm:w-5 sm:h-5 text-accent-tertiary flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="text-xs sm:text-sm font-medium text-dark-text-primary truncate">
                      {event.title}
                    </p>
                ...
                <div className="text-left sm:text-right flex-shrink-0">
                  <p className="text-xs sm:text-sm font-medium text-dark-text-primary">
                    {event.date}
                  </p>
