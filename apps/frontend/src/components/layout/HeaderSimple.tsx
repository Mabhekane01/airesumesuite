...
const FloatingNavigation = React.memo(() => {
  ...
  const authNavigation = [
    ...
    { name: "Resume", href: "/dashboard/resume/templates", icon: DocumentTextIcon, current: location.pathname.startsWith("/dashboard/resume") },
    ...
    { name: "Applications", href: "/dashboard/applications", icon: BriefcaseIcon, current: location.pathname.startsWith("/dashboard/applications") },
  ];
    ...
    <>
      ...
      <div className="fixed top-0 left-0 right-0 z-50 p-4">
        ...
        >
          ...
          <div className={`relative bg-gray-900/30 backdrop-blur-3xl border border-white/10 rounded-3xl transition-all duration-500 shadow-2xl ${
                        ...
                        <>
                          ...
                          <NotificationDropdown
                            notifications={notifications?.map((notification: any) => ({
                              ...
                              type: notification.type === "deadline" ? "deadline" : notification.type === "error" ? "warning" : notification.type || "info",
                              title: notification.title || "Notification",
                              message: notification.message || "",
                              timestamp: notification.createdAt || new Date().toISOString(),
                              read: Boolean(notification.read),
                              action: notification.action ? { label: notification.action.label || "View", href: notification.action.url || "#" } : undefined,
                            })) || []}
                              ...
                              {({ active }) => (
                                <Link
                                  ...
                                  className={`flex items-center space-x-3 w-full px-3 py-3 text-sm rounded-xl transition-all duration-200 ${
                                    active ? "bg-emerald-500/20 text-white border border-emerald-500/30" : "text-gray-300 hover:text-white"
                                  }`}
                                >
                            ...
                            <Menu.Item>
                              {({ active }) => (
                                <button
                                  ...
                                  className={`flex items-center space-x-3 w-full px-3 py-3 text-sm rounded-xl transition-all duration-200 ${
                                    active ? "bg-red-500/20 text-red-300 border border-red-500/30" : "text-gray-300 hover:text-red-300"
                                  }`}
                                >
