...
import { SearchFilters, SortOptions } from '../../utils/searchEngine';

interface JobApplicationSearchProps {
...
const SORT_OPTIONS = [
  ...
  { field: 'salary' as const, label: 'Salary' }
];

export default function JobApplicationSearch({ 
  onSearch, 
  searchSuggestions, 
  stats, 
  isLoading = false 
}: JobApplicationSearchProps) {
  ...
  const debouncedSearch = useCallback(
    debounce((searchQuery: string, searchFilters: SearchFilters, searchSort: SortOptions) => {
      console.log('🔍 JobApplicationSearch: Triggering search with:', { 
  ...
  useEffect(() => {
    console.log('🔍 JobApplicationSearch: useEffect triggered', { query, filters, sortOptions, totalApps: stats.totalApplications });
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by job title, company, location, status..."
            className="input-field-dark w-full pl-10 pr-20 text-sm"
          />
            ...
            <button
              ...
              className={`p-1.5 rounded-md transition-all duration-200 ${
                showFilters || activeFilterCount > 0
                  ...
                  : 'text-text-secondary hover:text-text-primary hover:bg-gray-700/60'
              }`}
            >
        ...
        <AnimatePresence>
          {showSuggestions && searchSuggestions.length > 0 && !query && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="absolute top-full left-0 right-0 mt-1 bg-gray-700/95 backdrop-blur-lg rounded-lg shadow-dark-xl border border-surface-200 z-50"
            >
              <div className="p-2">
                <p className="text-xs font-medium text-text-secondary uppercase tracking-wide mb-2 px-2">
                  Popular Searches
                </p>
                {searchSuggestions.map((suggestion, index) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-sm text-text-primary hover:bg-accent-primary/10 rounded-md transition-colors"
                  >
                    {suggestion}
                  </button>
                ))}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gray-700/60 rounded-lg p-3">
          ...
          <p className="text-lg font-semibold text-text-primary">{stats.totalApplications}</p>
        </div>
        <div className="bg-gray-700/60 rounded-lg p-3">
          <p className="text-xs text-text-secondary">Recent (30 days)</p>
          <p className="text-lg font-semibold text-accent-primary">{stats.recentApplications}</p>
        </div>
        <div className="bg-gray-700/60 rounded-lg p-3">
          <p className="text-xs text-text-secondary">Active Processes</p>
      ...
      <AnimatePresence>
        {showFilters && (
          <motion.div
            ...
            className="bg-gray-700/40 backdrop-blur-sm rounded-lg border border-surface-200 overflow-hidden"
          >
            <div className="p-6 space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-text-primary">Filters & Sorting</h3>
                    ...
                    <label
                      ...
                      className={`flex items-center space-x-2 p-3 rounded-lg cursor-pointer transition-all ${
                        filters.status?.includes(status.value)
                          ...
                          : 'bg-surface-50/60 hover:bg-surface-50/80 border border-transparent'
                      }`}
                    >
                      <input
                        type="checkbox"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-text-secondary mb-2">Sort Field</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs text-text-secondary mb-2">Direction</label>
                    <select
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-text-secondary mb-2">Location</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input
                      ...
                      className="rounded border-surface-200 bg-surface-50 text-accent-primary focus:ring-accent-primary focus:ring-offset-dark-secondary"
                    />
                    <span className="text-sm text-text-primary">Remote positions only</span>
                  </label>
                </div>
              </div>
            </div>
