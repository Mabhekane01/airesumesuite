    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-black text-text-tertiary uppercase tracking-widest">Repository Status</p>
            <p className="text-sm font-black text-brand-dark">{resumes.length} Architectures Detected</p>
          </div>
        </div>
        <button
          ...
          {resumes.map((resume, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group p-5 rounded-2xl cursor-pointer transition-all duration-300 border-2 relative overflow-hidden ${
                  isSelected
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex items-start justify-between">
                    <div className="min-w-0 flex-1">
                      <h3 className="text-base font-black text-brand-dark tracking-tight truncate group-hover:text-brand-blue transition-colors">
                    ...
                    {isSelected && (
                      <div className="w-6 h-6 rounded-lg bg-brand-blue text-white flex items-center justify-center shadow-lg">
                        <CheckCircleIcon className="w-4 h-4" />
                      </div>
                    )}
                  </div>

                  <div className="grid grid-cols-3 gap-2 py-3 border-y border-surface-100 group-hover:border-brand-blue/10 transition-colors">
                    <div className="text-center">
                      <p className="text-[10px] font-black text-brand-dark">{stats.workExpCount}</p>
                      <p className="text-[8px] font-black text-text-tertiary uppercase tracking-tighter">Nodes</p>
                    </div>
                    <div className="text-center border-x border-surface-100">
                      <p className="text-[10px] font-black text-brand-dark">{stats.skillsCount}</p>
                      <p className="text-[8px] font-black text-text-tertiary uppercase tracking-tighter">Skills</p>
                    </div>
                    <div className="text-center">
                      <p className="text-[10px] font-black text-brand-dark">{stats.completionScore}%</p>
                      <p className="text-[8px] font-black text-text-tertiary uppercase tracking-tighter">Health</p>
                    </div>
                  </div>

                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-text-tertiary font-bold text-[9px] uppercase tracking-widest">
