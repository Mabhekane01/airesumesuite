...
import { buildPdfServiceUrl, buildApiUrl } from '../../config/api';
...
const MergePDFTool: React.FC<MergePDFToolProps> = ({ activeFile }) => {
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      const response = await fetch(buildPdfServiceUrl('/api/pdf/editor/merge'), {
        method: 'POST',
        body: formData,
      });
          ...
          <div className="space-y-3">
            {files.map((pdfFile, index) => (
              <div
                ...
                className={`
                  flex items-center gap-4 p-4 bg-white/20 rounded-xl border border-gray-600/20
                  transition-all duration-200 cursor-move hover:bg-white/40 hover:border-gray-500/40
                  ${draggedIndex === index ? 'opacity-50 scale-95' : ''}
                `}
              >
                <div className="flex items-center gap-2 text-gray-400">
                  <GripVertical className="w-5 h-5" />
                  <span className="font-mono text-sm bg-white/40 text-white px-2 py-1 rounded">
                    {index + 1}
                  </span>
                </div>

                <div className="flex-shrink-0">
                  <div className="w-16 h-20 bg-white/30 border border-gray-600/30 rounded-lg shadow-lg flex items-center justify-center">
                    {pdfFile.url ? (
                      <Document
                        file={pdfFile.url}
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    disabled={index === 0}
                    className="p-2 rounded-lg bg-white/30 text-gray-400 hover:text-white hover:bg-white/50 disabled:opacity-50 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-white/30 text-red-400 hover:text-red-300 hover:bg-white/50 transition-colors"
                  >
