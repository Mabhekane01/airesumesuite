...
export default function ApiDebugInfo() {
  ...
  return (
    <div className="card-dark rounded-lg border border-surface-200 p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-text-primary">API Diagnostics</h2>
        <button
      ...
      {diagnostics && (
        <div className="space-y-4">
          <div className="bg-surface-50/20 rounded p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-surface-50/20 rounded p-4">
              <h4 className="font-medium text-text-primary mb-2">Server Health</h4>
              <div className="text-sm">
                <div className="flex items-center space-x-2">
                  <span className={`w-3 h-3 rounded-full ${diagnostics.serverHealth.isOnline ? 'bg-green-500' : 'bg-red-500'}`}></span>
                  <span className="text-text-secondary">{diagnostics.serverHealth.message}</span>
                </div>
              </div>
            </div>

            <div className="bg-surface-50/20 rounded p-4">
              <h4 className="font-medium text-text-primary mb-2">Authentication</h4>
              <div className="text-sm">
                <div className="flex items-center space-x-2">
            ...
            <div className="bg-surface-50/20 rounded p-4">
              <h4 className="font-medium text-text-primary mb-2">Job Applications API</h4>
              <div className="text-sm">
                <div className="flex items-center space-x-2">
                  <span className={`w-3 h-3 rounded-full ${diagnostics.jobApplicationsEndpoint.working ? 'bg-green-500' : 'bg-red-500'}`}></span>
                  <span className="text-text-secondary">{diagnostics.jobApplicationsEndpoint.message}</span>
                </div>
              </div>
            </div>

            <div className="bg-surface-50/20 rounded p-4">
              <h4 className="font-medium text-text-primary mb-2">Authenticated Request</h4>
              <div className="text-sm">
                <div className="flex items-center space-x-2">
                  <span className={`w-3 h-3 rounded-full ${diagnostics.authenticatedRequest.working ? 'bg-green-500' : 'bg-red-500'}`}></span>
                  <span className="text-text-secondary">{diagnostics.authenticatedRequest.message}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mt-6 text-sm text-dark-text-muted">
        <p>
          ...
          <code className="bg-surface-50/20 px-2 py-1 rounded">window.debugAPI.runDiagnostics()</code>
        </p>
      </div>
    </div>
  );
