...
export default function ConversationalCoverLetterBuilder() {
  ...
  const processConversation = async (input: string) => {
    switch (currentStep) {
      case 'welcome': await handleJobInputStep(input); break;
      case 'job-input': await handleJobDetailsInput(input); break;
      case 'resume-selection': await handleResumeSelection(input); break;
      case 'generation': await handleGenerationFeedback(input); break;
      case 'review': await handleReviewFeedback(input); break;
      case 'editing': await handleEditingInput(input); break;
      default: await handleGeneralInput(input);
    }
  };
  ...
  const handleJobInputStep = async (input: string) => {
    ...
    if (jobTitleMatch) setCoverLetterData(prev => ({ ...prev, jobTitle: jobTitleMatch[1].trim() }));
    if (companyMatch) setCoverLetterData(prev => ({ ...prev, companyName: companyMatch[1].trim() }));
    ...
    addMessage({
      ...
      suggestions: [ "I have a job posting URL", "I'll provide the details manually", "Let me paste the job description" ],
      actions: [ { label: "Analyze Job URL", action: () => setCurrentStep('job-input'), variant: 'primary' } ]
    });
  ...
  const handleJobDetailsInput = async (input: string) => {
    if (input.toLowerCase().includes('url') || input.includes('http')) {
      ...
      if (urlMatch) { await handleJobUrlAnalysis(urlMatch[1]); return; }
    }
    if (input.toLowerCase().includes('manual') || input.toLowerCase().includes('details')) {
      addMessage({
        ...
        suggestions: ["Software Engineer", "Product Manager", "Data Scientist", "Marketing Specialist"]
      });
      return;
    }
  ...
  const handleJobUrlAnalysis = async (jobUrl: string) => {
    ...
    addMessage({ type: 'ai', content: "Analyzing the job posting... This will just take a moment." });
    ...
    try {
      ...
      addMessage({
        ...
        actions: [ { label: "View Full Analysis", action: () => showJobAnalysis(jobData) } ]
      });
      await proceedToResumeSelection();
    } catch (error) {
      addMessage({
        ...
        suggestions: [ "Let me try a different URL", "I'll provide details manually" ]
      });
    } finally {
  ...
  const proceedToResumeSelection = async () => {
    if (resumes.length === 0) {
      addMessage({
        ...
        suggestions: [ "Continue without resume", "Upload a resume first" ],
        actions: [ { label: "Generate Cover Letter", action: () => startGeneration(), variant: 'primary' } ]
      });
    } else {
  ...
  const handleResumeSelection = async (input: string) => {
    ...
    if (selectedResume) {
      ...
      addMessage({
        ...
        suggestions: [ "Professional and formal", "Enthusiastic and passionate", "Casual and conversational", "Conservative and traditional" ]
      });
    } else {
      addMessage({
        type: 'ai',
    ...
    try {
      if (coverLetterData.resumeId && coverLetterData.jobDescription) {
        ...
        if (variations.length > 0) {
          setCoverLetterData(prev => ({ ...prev, content: variations[0].content, tone: variations[0].tone as any }));
          addMessage({
            type: 'ai',
            ...
            actions: [
              { label: "Review Cover Letter", action: () => setCurrentStep('review'), variant: 'primary' },
              { label: "Try Different Tone", action: () => showToneOptions() }
            ]
          });
      ...
      addMessage({
        ...
        suggestions: [ "Try again", "Build it manually", "Start over" ]
      });
    } finally {
      setIsGenerating(false);
    }
  ...
  const generateBasicCover = async () => {
    ...
    if (basicContent.success && basicContent.content) {
      ...
      addMessage({
        ...
        actions: [ { label: "Review Cover Letter", action: () => setCurrentStep('review'), variant: 'primary' } ]
      });
      setCurrentStep('review');
    }
  };
  ...
  const handleReviewFeedback = async (input: string) => {
    if (input.toLowerCase().includes('looks good') || input.toLowerCase().includes('approve')) {
      ...
      addMessage({
        ...
        suggestions: [
          "Make it more enthusiastic", "Add more technical details", "Emphasize leadership experience", "Make it more concise"
        ]
      });
    } else {
  ...
  const handleEditingInput = async (input: string) => {
    ...
    try {
      ...
      addMessage({
        ...
        actions: [
          { label: "Looks Great!", action: () => proceedToFinalization() },
          { label: "More Changes", action: () => setCurrentStep('editing') }
        ]
      });
    } catch (error) {
      addMessage({ type: 'ai', content: "I had trouble making that change. Could you be more specific about what you'd like to adjust?" });
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  const proceedToFinalization = async () => {
    ...
    addMessage({
      ...
      actions: [
        { label: "Download PDF", action: () => downloadCoverLetter('pdf'), variant: 'primary' },
        { label: "Save to Dashboard", action: () => saveCoverLetter() },
  ...
  const handleGeneralInput = async (input: string) => {
    addMessage({
      ...
      suggestions: [ "Improve current cover letter", "Start a new cover letter", "Ask a question" ]
    });
  };
  ...
  const handleGenerationFeedback = async (input: string) => {
    if (input.toLowerCase().includes('stop') || input.toLowerCase().includes('cancel')) {
      setIsGenerating(false);
      addMessage({ type: 'ai', content: "Generation stopped. Would you like to try a different approach?" });
    }
  };
  ...
  const downloadCoverLetter = async (format: 'pdf' | 'docx' | 'txt') => {
    try {
      ...
      addMessage({ type: 'ai', content: `📥 Your cover letter has been downloaded as a ${format.toUpperCase()} file! Anything else I can help you with?` });
    } catch (error) {
      toast.error('Download failed. Please try again.');
      addMessage({ type: 'ai', content: "Sorry, there was an issue downloading your cover letter. Would you like to try a different format or save it to your dashboard instead?" });
    }
  };
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        addMessage({
          ...
          actions: [ { label: "View in Dashboard", action: () => navigate(`/dashboard/cover-letters/${result.data!._id}`) } ]
        });
      }
    } catch (error) {
      toast.error('Failed to save cover letter');
      addMessage({ type: 'ai', content: "I had trouble saving your cover letter. Would you like to try downloading it instead?" });
    }
  };
  ...
  const copyToClipboard = async () => {
    try {
      ...
      addMessage({ type: 'ai', content: "📋 Your cover letter has been copied to your clipboard! You can now paste it anywhere you need it." });
    } catch (error) {
  ...
  return (
    <PremiumFeatureGate 
      feature="AI Conversational Cover Letter Builder" 
            ...
            <div className="lg:col-span-2">
              <Card className="card-dark h-[600px] flex flex-col">
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {message.suggestions.map((suggestion, idx) => (
                              <button
                                ...
                                className="px-3 py-1 text-sm bg-dark-accent/10 hover:bg-dark-accent/20 rounded-full border border-surface-200 transition-colors"
                              >
                                {suggestion}
                              </button>
                            ))}
                ...
                {currentSuggestions.length > 0 && (
                  <div className="px-6 py-2 border-t border-surface-200">
                  ...
                  <div className="space-y-2 text-sm">
                    {coverLetterData.jobTitle && (
                      <div>
                        ...
                        <div className="text-text-primary font-medium">{coverLetterData.jobTitle}</div>
                      </div>
                    )}
                    {coverLetterData.companyName && (
                      <div>
                        <span className="text-dark-text-muted">Company:</span>
                        <div className="text-text-primary font-medium">{coverLetterData.companyName}</div>
                      </div>
                    )}
                    {coverLetterData.tone && (
                      <div>
                        <span className="text-dark-text-muted">Tone:</span>
                        <div className="text-text-primary font-medium capitalize">{coverLetterData.tone}</div>
                      </div>
                    )}
                  </div>
                </Card>
              ...
              <Card className="card-dark max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-text-primary">Cover Letter Preview</h2>
                    <div className="flex space-x-2">
                      <Button
