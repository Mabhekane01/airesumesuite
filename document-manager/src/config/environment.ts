...
export interface Config {
  ...
  AI_RESUME_SUITE_API_KEY?: string;
...
const validateConfig = (): Config => {
  ...
  return {
    ...
    ALLOWED_FILE_TYPES: (process.env.ALLOWED_FILE_TYPES || 'pdf,doc,docx,ppt,pptx,txt,jpg,jpeg,png,gif,webp').split(','),
    
    // CORS
    CORS_ORIGINS: (process.env.CORS_ORIGINS || 'http://localhost:3000,http://localhost:5173').split(','),
    
    // Document Sharing Service
    DOCUMENT_SHARING_SERVICE_URL: process.env.DOCUMENT_SHARING_SERVICE_URL || 'http://localhost:4000',
    
    // AI Resume Suite Integration
    AI_RESUME_SUITE_URL: process.env.AI_RESUME_SUITE_URL || 'http://localhost:3000',
    ...
    // Webhooks
    WEBHOOK_RETENTION_DAYS: parseInt(process.env.WEBHOOK_RETENTION_DAYS || '90'),
