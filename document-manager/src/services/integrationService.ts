...
export class AIResumeSuiteIntegration {
  ...
  static async notifyDocumentCreated(documentData: {
    ...
    try {
      ...
      await axios.post(`${config.AI_RESUME_SERVICE_URL}/documents/created`, {
  ...
  static async getUserDocuments(userId: string, authToken: string): Promise<any[]> {
    try {
      const response = await axios.get(`${config.AI_RESUME_SERVICE_URL}/resumes`, {
        headers: {
  ...
  static async importDocument(userId: string, authToken: string, resumeId: string): Promise<string | null> {
    try {
      // Get resume PDF from AI Resume Suite
      const response = await axios.get(`${config.AI_RESUME_SERVICE_URL}/resumes/${resumeId}/pdf`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
        },
        responseType: 'stream',
...
export class PDFEditorIntegration {
  ...
  static async sendForEditing(documentData: {
    id: string;
    title: string;
    fileUrl: string;
    userId: string;
    ...
    try {
      const response = await axios.post(`${config.PDF_EDITOR_SERVICE_URL}/edit/import`, {
        documentId: documentData.id,
        title: documentData.title,
        fileUrl: documentData.fileUrl,
  ...
  static async receiveEditedDocument(editSessionId: string, editedFileData: any): Promise<string | null> {
    try {
...
export class DocumentManagerAPI {
  ...
  static async createFromExternalService(data: {
    ...
    try {
      ...
      await WebhookService.sendEvent('document.created', {
        documentId,
        title: data.title,
        source: data.source,
        userId: data.userId
  ...
  static async getUserDocumentsForService(userId: string, serviceType: string): Promise<any[]> {
    try {
  ...
  static async shareWithService(documentId: string, serviceType: string, permissions: any): Promise<string | null> {
    try {
      ...
      await cache.set(`temp_access:${accessToken}`, {
        documentId,
        serviceType,
        permissions,
...
export class IntegrationWebhooks {
    ...
    try {
      switch (event) {
        case 'document.edited':
          await PDFEditorIntegration.receiveEditedDocument(data.sessionId, data.file);
          break;
  ...
  private static async handleResumeCreated(data: any): Promise<void> {
    ...
    logger.info('AI Resume Suite resume created', { resumeId: data.resumeId, userId: data.userId });
  }
  
  private static async handleCoverLetterCreated(data: any): Promise<void> {
    // Auto-import cover letter into document manager
    logger.info('AI Resume Suite cover letter created', { coverLetterId: data.coverLetterId, userId: data.userId });
  }
  ...
  private static async handleEditingCompleted(data: any): Promise<void> {
    ...
    logger.info('PDF editing completed', { sessionId: data.sessionId, userId: data.userId });
  }
}
